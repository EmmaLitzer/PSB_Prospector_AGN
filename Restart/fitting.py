...
def fit_model(obs, model, sps, result=None, noise=(None, None), lnprobfn=lnprobfn,
    ...
    output["sampling"] = run_sampler(obs, model, sps, noise, result,
                                     lnprobfn=lnprobfn, **kwargs)
            # Add result
    return output
...
def run_emcee(obs, model, sps, noise, result=None, lnprobfn=lnprobfn,
    ...
    """
    if result != None:
        q = result['bestfit']['parameter']
        print('q is \t', q)
    else:
        q = model.theta.copy()
        print("\tFirst Run. No result detected")
    print('model.theta is \t', model.theta.copy())
    ...
    if initial_positions is not None:
        ...
        t = time.time()
        out = meth(lnprobfn, q, model, result, hdf5=hfile,
                   postkwargs=postkwargs, **kwargs)
